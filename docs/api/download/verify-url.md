...
**Success Response - Document (200):**

```json
{
  "urlType": "document",
  "fileSize": 1548576,
  "fileSizeFormatted": "1.48 MB",
...
|---------------------|--------|-----------------------------------------------------------------------------------|
| `urlType`           | string | Detected URL type: `youtube`, `streaming`, `direct-file`, `document`, or `invalid` |
...
| `mimeType`          | string | Content MIME type (optional, for direct files and documents)                      |
| `documentType`      | string | Document type: `pdf`, `png`, `jpg`, `tiff`, `txt`, or `docx` (only for documents) |
...
**Document (`document`):**
- Extensions: `.pdf`, `.png`, `.jpg`, `.jpeg`, `.tiff`, `.tif`, `.txt`, `.docx`
- Metadata: `curl` for headers (file size, MIME type)
- Maximum size: 300MB
- Returns `documentType` field indicating specific document format
...
```bash
curl -X POST http://localhost:4321/api/download/verify-url \
