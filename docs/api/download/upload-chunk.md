...
- [Chunk Assembly Process](#chunk-assembly-process)
- [Notes](#notes)
...
```json
{
  "error": "Invalid chunk: expected File"
}
```
...
| `duration`       | number  | Duration in seconds (only when complete, optional)        |
...
```bash
# Upload chunk 0
curl -X POST http://localhost:4321/api/download/upload-chunk \
  -F "chunk=@chunk_0.bin" \
  -F "chunkIndex=0" \
  -F "totalChunks=10" \
  -F "fileId=abc-123" \
