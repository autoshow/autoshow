...
test('process audio with groq', async ({ page, request }) => {
  ...
  await page.click('button:has-text("GPT OSS 20B")')
  ...
  const report = {
    ...
    tests: [
      {
        testName: 'process audio with groq',
        status: 'passed',
        timestamps: {
  ...
  const reportPath = path.join(logsDir, `${timestamp}-playwright-groq-short-summary-report.json`)
  const serverLogsPath = path.join(logsDir, `${timestamp}-playwright-groq-short-summary-server.log`)
  const pollLogsPath = path.join(logsDir, `${timestamp}-playwright-groq-short-summary-poll.log`)
