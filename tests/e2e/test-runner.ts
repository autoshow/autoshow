...
import type { TranscriptionConfig, TestSuiteConfig } from "./test-configs"
...
function buildTestName(config: E2ETestConfig): string {
  const typeLabel = config.suite.type === "local" ? "local file" : "direct URL"
  return `${config.suite.prefix}: processes ${typeLabel} with ${config.transcription.nameSuffix} and GPT-5-nano short summary`
}

function buildReportSuffix(config: E2ETestConfig): string {
  const typeLabel = config.suite.type === "local" ? "local" : "direct"
  return `${config.suite.prefix}-${config.transcription.id}-${typeLabel}-${config.transcription.nameSuffix}-gpt-5-nano-short-summary`
}
...
async function buildFormData(config: E2ETestConfig, baseUrl: string): Promise<FormDataResult> {
  ...
  formData.append("transcriptionOption", config.transcription.transcriptionOption)
  formData.append("transcriptionModel", config.transcription.transcriptionModel)
  formData.append("selectedPrompts", DEFAULT_PROMPTS)
  formData.append("llmService", DEFAULT_LLM_SERVICE)
  formData.append("llmModel", DEFAULT_LLM_MODEL)
...
function buildTestResult(params: TestResultParams): TestResult {
  return {
    ...
    input: {
      ...
      transcriptionOption: params.config.transcription.transcriptionOption,
      transcriptionModel: params.config.transcription.transcriptionModel,
...
export function createE2ETest(config: E2ETestConfig) {
