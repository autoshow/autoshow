...
export const runGroqStructured = async (
  prompt: string,
  model: string,
  ...
  progressTracker?.updateStepProgress(stepNumber, 30, `Calling Groq ${model}...`)
  ...
  try {
    ...
    logLLMCompletion('Groq', metadata, progressTracker, stepNumber)
    return { response: parsed, metadata }
  } catch (error) {
    return handleLLMError(error, 'Groq', progressTracker, stepNumber)
  }
}
