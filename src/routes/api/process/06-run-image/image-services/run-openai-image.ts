...
export const runOpenAIImage = async (
  ...
  l(`Starting ${SERVICE_NAME} image generation`, { model, dimensions: dimensionOrRatio })
  ...
  try {
    for (let i = 0; i < selectedPrompts.length; i++) {
      ...
      const response = await client.images.generate({
        model,
        prompt,
        size: dimensionOrRatio as '1024x1024' | '1536x1024' | '1024x1536',
        quality: 'high'
      })
      ...
      const cost = getCostPerImage('openai', model, dimensionOrRatio)
    ...
    const metadata = buildImageMetadata('openai', model, startTime, results, selectedPrompts)
