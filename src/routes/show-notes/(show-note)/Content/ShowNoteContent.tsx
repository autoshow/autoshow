...
type Props = {
  ...
  formatImagePromptName: (promptId: string) => string
  formatVideoPromptName: (promptId: string) => string
  ...
  return (
    <>
      <section class={s.section}>
        ...
        <div 
          class={s.summaryContent}
          innerHTML={props.renderMarkdown(props.note.summary)}
        />
      </section>
              ...
              {(imageFile) => {
                ...
                return (
                  <div class={s.imageCard}>
                    <img 
                      src={props.getAudioUrl(props.note.id, imageFile)}
                      alt={props.formatImagePromptName(promptType)}
                      class={s.generatedImage}
                    />
                    <p class={s.imageCaption}>
                      {props.formatImagePromptName(promptType)}
                    </p>
                  </div>
                )
              }}
      ...
      <Show when={props.note.music_gen_enabled && props.note.music_gen_file}>
        <section class={s.section}>
          ...
          <div class={s.musicPlayer}>
            ...
            <div class={s.audioMeta}>
              <span>Service: {SERVICES_CONFIG.music.elevenlabs.name}</span>
              ...
              {(videoFile) => {
                ...
                return (
                  <div class={s.videoCard}>
                    ...
                    >
                      <source 
                        src={props.getAudioUrl(props.note.id, videoFile)} 
                        type="video/mp4"
                      />
                      Your browser does not support the video element.
                    </video>
                    <div class={s.videoCaption}>
                      <p class={s.videoCaptionTitle}>{props.formatVideoPromptName(promptType)}</p>
